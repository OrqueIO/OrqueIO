...
var pagesModule = angular.module('orqueio.common.pages', ['ngRoute']);
...
var ResponseErrorHandlerInitializer = [
  ...
  function(
    ...
    function handleHttpError(event, error) {
      ...
      // avoid displaying any error message when performing a request against /orqueio-welcome
      // since the get request against it is only necessary to determine if deployed
      const config = error.response.config;
      if (config.method === 'GET' && config.url === '/orqueio-welcome') {
        return;
      }
