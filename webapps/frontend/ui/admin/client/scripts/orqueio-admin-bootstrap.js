...
window.bust = ORQUEIO_VERSION; // eslint-disable-line
window.DOMPurify = require('dompurify');

// orqueio-admin-bootstrap is copied as-is, so we have to inline everything
...
requirejs.config({
  ...
  urlArgs: `bust=${ORQUEIO_VERSION}` // eslint-disable-line
});
...
define('orqueio-admin-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var orqueioAdminUi = require('./orqueio-admin-ui');
    orqueioAdminUi.exposePackages(window);

    requirejs([`${appRoot}/lib/globalize.js`], function(globalize) {
      globalize(
        requirejs,
        ['angular', 'orqueio-commons-ui', 'orqueio-bpm-sdk-js', 'jquery'],
        window
      );
      ...
      pluginPackages.forEach(function(plugin) {
        ...
        node.setAttribute(
          'href',
          plugin.location + `/plugin.css?bust=${ORQUEIO_VERSION}` // eslint-disable-line
        );
      ...
      requirejs(dependencies, function(angular) {
        ...
        if (typeof config !== 'undefined' && config.requireJsConfig) {
          ...
          requirejs(custom.deps || [], function() {
            ...
            // initialize Admin
            orqueioAdminUi.init(pluginDependencies);
          });
        } else {
...
requirejs(['orqueio-admin-bootstrap'], function() {});
