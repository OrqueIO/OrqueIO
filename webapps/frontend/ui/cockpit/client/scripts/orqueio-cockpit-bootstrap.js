...
//  orqueio-cockpit-bootstrap is copied as-is, so we have to inline everything
...
define('orqueio-cockpit-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var orqueioCockpitUi = require('./orqueio-cockpit-ui');
    orqueioCockpitUi.exposePackages(window);
    ...
    requirejs([`${appRoot}/lib/globalize.js`], function(globalize) {
      ...
      requirejs(dependencies, function(jquery, angular) {
        ...
        function initCockpitUi(pluginDependencies) {
          ...
          // initialize Cockpit
          orqueioCockpitUi.init(pluginDependencies);
        }
      });
    });
  };
...
requirejs(['orqueio-cockpit-bootstrap'], function() {});
