...
import io.orqueio.bpm.model.bpmn.OrqueioExtensionsTest;
 ...
 * {@link OrqueioExtensionsTest} this test uses directly the get*Ns() methods to
...
public class CompatabilityTest {
  ...
  public void modifyingElementWithActivitiNsKeepsIt() {
    BpmnModelInstance modelInstance = Bpmn.readModelFromStream(OrqueioExtensionsTest.class.getResourceAsStream("OrqueioExtensionsCompatabilityTest.xml"));
    ...
    Collection<OrqueioExecutionListener> listeners = extensionElements.getChildElementsByType(OrqueioExecutionListener.class);
    String listenerClass = "org.foo.Bar";
    for (OrqueioExecutionListener listener : listeners) {
      listener.setOrqueioClass(listenerClass);
    }
    for (OrqueioExecutionListener listener : listeners) {
  ...
  public void modifyingAttributeWithActivitiNsKeepsIt() {
    ...
    process.setOrqueioJobPriority(priority);
    process.setOrqueioTaskPriority(priority);
    Integer historyTimeToLive = 10;
    process.setOrqueioHistoryTimeToLive(historyTimeToLive);
    process.setOrqueioIsStartableInTasklist(false);
    process.setOrqueioVersionTag("v1.0.0");
    ...
    assertThat(process.isOrqueioStartableInTasklist(), is(false));
    assertThat(process.getOrqueioVersionTag(), is("v1.0.0"));
  }
