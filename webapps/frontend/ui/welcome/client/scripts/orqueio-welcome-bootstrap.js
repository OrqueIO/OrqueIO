...
// orqueio-welcome-bootstrap is copied as-is, so we have to inline everything
...
define('orqueio-welcome-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var orqueioWelcomeUi = require('./orqueio-welcome-ui');
    orqueioWelcomeUi.exposePackages(window);
      ...
      requirejs(dependencies, function(angular) {
        ...
        if (typeof config !== 'undefined' && config.requireJsConfig) {
          ...
          requirejs(custom.deps || [], function() {
            ...
            // initialize Welcome
            orqueioWelcomeUi.init(pluginDependencies);
          });
        } else {
...
requirejs(['orqueio-welcome-bootstrap'], function() {});
