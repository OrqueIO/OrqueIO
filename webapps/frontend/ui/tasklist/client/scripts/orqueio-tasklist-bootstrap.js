...
//  orqueio-tasklist-bootstrap is copied as-is, so we have to inline everything
...
define('orqueio-tasklist-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var orqueioTasklistUi = require('./orqueio-tasklist-ui');
    orqueioTasklistUi.exposePackages(window);
      ...
      requirejs(dependencies, function(angular) {
        ...
        if (typeof config !== 'undefined' && config.requireJsConfig) {
          ...
          requirejs(custom.deps || [], function() {
            ...
            orqueioTasklistUi.init(pluginDependencies);
          });
        } else {
...
requirejs(['orqueio-tasklist-bootstrap'], function() {});
