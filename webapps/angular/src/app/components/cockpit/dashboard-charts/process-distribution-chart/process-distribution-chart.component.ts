...
@Component({
  selector: 'app-process-distribution-chart',
  standalone: true,
    ...
    >
      <div class="chart-container">
        ...
        <div class="chart-center">
          <span class="chart-center__value" [style.color]="isHovering && hoveredColor ? hoveredColor : defaultColor">{{ isHovering ? hoveredValue : totalInstances }}</span>
      ...
      <div class="chart-label" chart-footer [class.chart-label--hover]="isHovering" [style.color]="isHovering && hoveredColor ? hoveredColor : null">
        {{ isHovering ? hoveredLabel : ('cockpit.dashboard.runningProcesses' | translate) }}
      </div>
    </app-chart-card>
  `,
  styles: [`
...
export class ProcessDistributionChartComponent implements OnInit, OnDestroy {
  ...
  private subscribeToStore(): void {
    // Get actual running instances count from dashboard stats
    this.store.select(DashboardSelectors.selectDashboardStats)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(stats => {
        if (stats) {
          this.totalInstances = stats.runningProcessInstances;
          this.cdr.markForCheck();
        }
      });

    this.store.select(DashboardSelectors.selectProcessDistribution)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(processes => {
    ...
    this.store.select(DashboardSelectors.selectProcessDistributionLoading)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(loading => {
    ...
    this.store.select(DashboardSelectors.selectProcessDistributionError)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(error => {
        this.error = error;
        this.cdr.markForCheck();
  ...
  private updateChart(): void {
    ...
    const hasData = this.processes.length > 0;
    ...
    // If no process distribution data, show empty gray doughnut with totalInstances in center
    if (!hasData) {
      const emptyData = [1];
    ...
    // Display all processes (like AngularJS)
    const displayData = this.processes;
    const labels = displayData.map(p => p.name);
    const data = displayData.map(p => p.instanceCount);
      ...
      options: {
        ...
        onHover: isEmpty ? undefined : (_event, elements) => {
          if (elements.length > 0) {
            ...
            if (process) {
              ...
              this.hoveredValue = process.instanceCount;
              this.hoveredLabel = process.name;
              this.hoveredPercentage = process.percentage?.toFixed(1) || '0';
        ...
        onClick: isEmpty ? undefined : (_event, elements) => {
          if (elements.length > 0) {
            ...
            const process = this.processes[index];
            if (process) {
              this.navigateToProcess(process.key);
            }
          }
        }
      }
  ...
  onRefresh(): void {
    this.store.dispatch(DashboardActions.loadProcessDistribution());
  }
}
